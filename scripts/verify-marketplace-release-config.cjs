...
function verifyProductionInputs({ configPath = defaultConfigPath, environment = process.env } = {}) {
...
function selfTest() {
  ...
  try {
    ...
    const verified = verifyProductionInputs({ configPath: validPath, environment: { CSC_LINK: 'fixture' } });
    ...
    assert.throws(() => verifyProductionInputs({ configPath: unprovisionedPath, environment: { CSC_LINK: 'fixture' } }), /production Marketplace requires/);
    console.log('Production release input gate self-tests passed.');
    console.log('- Missing config, unprovisioned catalog, and missing code-signing certificate fail closed.');
  } finally {
