...
| --- | --- | --- | --- |
| `virtual-robot-arm` | `/models/real-devices/ur5e-real-open-source.glb` | Universal Robots ROS2 Description, UR5e collision STL meshes | BSD-3-Clause |
...
The pinned upstream Universal Robots package README and `package.xml` list
UR8LONG, UR15, UR18, UR20, and UR30 graphical meshes under separate Graphical
Documentation terms, while stating that all other repository content is
BSD-3-Clause. The redistributed arm model is UR5e, so its source meshes fall in
the BSD-3-Clause scope. The source commit, scope evidence, runtime digest, and
license digest are recorded in `third_party/model-assets/provenance.json`; the
license text is included in `THIRD_PARTY_NOTICES`. This is not a general
conclusion about other UR models.
...
- Universal Robots ROS2 Description: official description files and meshes for Universal Robots manipulators. Some listed models use separate graphical documentation terms; verify the exact model scope before importing anything other than the reviewed UR5e asset.
