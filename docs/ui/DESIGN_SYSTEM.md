...
- 用户取消系统文件选择器不属于错误，不显示失败通知；真正的失败使用持久 `error` 通知，保留原始错误详情并提供与原操作一致的“重试”入口。
- 浏览器 Open 必须使用真实、可重复触发的隐藏文件输入；每次打开选择器前清空旧值，以允许用户修正后重新选择同名文件。
- 重试不得改变工程、工作区、安全状态或执行模式；只有操作成功后才显示成功通知。导出失败不得伪装成已下载。
- 资产导入与手册导入关闭后必须回到仍然可见的 File 菜单按钮（不能回到已经收起的隐藏菜单项）；Escape 与 Tab/Shift+Tab 契约和其他模态一致。
