...
- **Windows 安装生命周期闭环 ✅**：`desktop:pack` 在验包与未安装态 renderer smoke 后，于专用临时目录执行当前用户静默干净安装、安装态首启、强制断网显式规则编译器降级、原位重装、卸载载荷/登记清零与用户数据保留；发现任意既有 RealityWarden 安装登记会默认拒绝，绝不覆盖用户安装。生命周期清单与总发布证据均带 SHA256，总证据 schema v2 仅在全部门禁通过后写出；历史版本迁移、签名与真机仍明确不在本记录声明范围。2026-07-15 实际安装包 `186418387` bytes，SHA256 `BDBC503FDA26C2DDD5BFBC66747679EFB22500E6E1A8F0B83F5AD9735C62A4F4`，完整生命周期实跑通过。

## 下一步

1. **最终软件/产品设计验收矩阵**：集中收口 1440×900 与 1180×720、Windows 缩放、长中英文、键盘遍历、高对比度、复杂对话框和错误恢复的可复现视觉/交互证据；不得覆盖尚未提交的资产预览健壮性改动，不修改业务或安全逻辑。
