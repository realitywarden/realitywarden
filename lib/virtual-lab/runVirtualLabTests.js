...
function main() {
  ...
  const realAdapterSource = fs.readFileSync(path.join(root, 'lib', 'adapter', 'RealDeviceAdapter.ts'), 'utf8');
  const transportSource = fs.readFileSync(path.join(root, 'lib', 'adapter', 'DeviceTransport.ts'), 'utf8');
  assert(realAdapterSource.includes('implements AdapterInterface'), 'RealDeviceAdapter must implement AdapterInterface');
  assert(transportSource.includes('send(command: AdapterCommand)'), 'DeviceTransport must expose command transport');
  ...
  console.log(`Virtual Lab tests passed: ${profileIds.length} profiles, ${scenarios.length} scenarios, shared AdapterInterface verified.`);
}
