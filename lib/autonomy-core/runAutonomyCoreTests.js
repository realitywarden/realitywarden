...
assert(page.includes('new AutonomyCore().run') && page.includes("autonomyResult.status !== 'execute'") && page.includes('no device commands generated'), 'UI Run must pass through AutonomyCore and stop non-execute statuses.');
assert.equal((page.match(/new AutonomyCore\(\)\.run/g) ?? []).length, 1, 'AutonomyCore.run must only be called by explicit Run, never by render/preview.');
assert(page.includes('new LiveScenarioRunner().run(runProfile, runScenarioDefinition, runPrompt, autonomyResult?.task_dsl)'), 'UI Run must pass AutonomyCore TaskDSL into LiveScenarioRunner.');
assert(page.indexOf('new AutonomyCore().run(runPrompt') < page.indexOf('new LiveScenarioRunner().run(runProfile'), 'UI Run must execute AutonomyCore before LiveScenarioRunner dispatch.');
