...
assert(productionReleaseGate.includes('productionRequired: true') && productionReleaseGate.includes('WIN_CSC_LINK') && productionReleaseGate.includes('CSC_LINK'), 'production release gate must require authoritative distribution validation and a Windows signing certificate.');
...
for (const ignoredPath of ['/release/', 'demo/', '.next-build/', 'dist-electron/', '.env', 'marketplace/distribution.json']) {
