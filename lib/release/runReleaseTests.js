...
const releaseNotes = read('docs/RELEASE_NOTES_V0.3.0.md');
const releaseReadiness = read('docs/RELEASE_READINESS_V0.3.0.md');
...
assert.equal(version, '0.3.0', 'This release closure must remain pinned to v0.3.0.');
...
assert(readme.includes('39/39 passing') && readme.includes('5/5'), 'README must publish current automated safety evidence counts.');
...
for (const document of [releaseNotes, releaseReadiness]) {
  ...
  assert(document.includes('39/39') && document.includes('5/5'), 'Release documents must carry current automated safety evidence.');
