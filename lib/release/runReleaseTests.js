...
for (const document of [releaseNotes, releaseReadiness]) {
  ...
  assert(document.includes('43/43') && document.includes('5/5') && document.includes('12/12'), 'Release documents must carry current automated safety and manual-import evidence.');
...
assert(desktopApp.includes('automatically runs the unpacked install-layout executable'), 'Desktop packaging docs must promise only the automatic smoke behavior the script implements.');
