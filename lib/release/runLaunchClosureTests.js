...
assert(verifyIndex >= 0 && verifyIndex < smokeIndex && smokeIndex < lifecycleIndex && lifecycleIndex < evidenceIndex, 'Packaging must verify contents, pass renderer smoke and installation lifecycle, then write evidence in that order.');
...
try {
  ...
  assert.deepEqual(manifest.gates.map((gate) => gate.status), ['passed', 'passed', 'passed']);
