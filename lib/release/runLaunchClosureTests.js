...
assert(verifyIndex >= 0 && verifyIndex < smokeIndex && smokeIndex < designIndex && designIndex < lifecycleIndex && lifecycleIndex < evidenceIndex, 'Packaging must verify contents, pass renderer smoke, design acceptance, and installation lifecycle, then write evidence in that order.');
...
try {
  ...
  const fixtureLifecycle = {
    ...
    gates: { clean_install: 'passed', installed_first_run: 'passed', offline_degradation: 'passed', in_place_reinstall: 'passed', uninstall: 'passed' },
  ...
  const fixtureDesign = {
    ...
    dialogs: ['action-composer', 'asset-import', 'manual-import'].map((id) => ({ id, status: 'passed', focusRestored: true })),
  ...
  assert.deepEqual(manifest.gates.map((gate) => gate.status), ['passed', 'passed', 'passed', 'passed']);
