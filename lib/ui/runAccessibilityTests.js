...
const initialAutosaveRestore = page.slice(page.indexOf("const rawWorkspace = window.localStorage.getItem(workspaceStorageKey);"), page.indexOf('// Debounced autosave:'));
assert(initialAutosaveRestore.includes('setAutosaveQuarantined(true)') && !initialAutosaveRestore.includes('localStorage.removeItem'), 'Startup must quarantine a corrupt autosave instead of silently deleting it.');
assert(page.includes('if (autosaveQuarantined) return;') && page.includes('setAutosaveQuarantined(false)'), 'Autosave must pause while corrupt data awaits an explicit operator decision.');
