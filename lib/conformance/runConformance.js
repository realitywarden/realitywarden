...
function main() {
  ...
  assert(manualImportWizard.includes('<SemanticDeviceStage') && manualImportWizard.includes("reviewMode === 'compare'") && manualImportWizard.includes('Second gate: enable in Virtual Lab'), 'Manual review must provide semantic geometry, source comparison, and a distinct second enablement gate.');
  ...
  assert(page.includes('restoreEnabledManualSimulationAsset') && page.includes('Manual devices without valid simulation-only enablement were removed from the workspace.'), 'Project restore must reject invalid manual enablement instead of silently falling back to another profile.');
  assert(manualImportWizard.includes("event.key === 'Escape'") && manualImportWizard.includes("event.key !== 'Tab'") && globalStyles.includes('.manual-import-active > :not([data-manual-import-modal])'), 'Manual import must provide modal keyboard containment and prevent Three.js overlay bleed-through.');
