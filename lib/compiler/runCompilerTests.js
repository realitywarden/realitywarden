...
assert(
  page.includes('const runPrompt = manifestRun ? manifestRun.label : (prompt.trim() || getLocalizedPrompt(runScenarioDefinition, language))'),
  'Normal runs must preserve the user typed prompt while manifest runs retain their validated action label.'
);
assert(
  /new LiveScenarioRunner\(\)\.run\(\s*runProfile,\s*runScenarioDefinition,\s*runPrompt,/.test(page),
  'LiveScenarioRunner must receive the resolved runPrompt instead of silently replacing user input with scenario defaults.'
);
